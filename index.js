...
const verifyToken = async (req, res, next) => {
    ...
    console.log('tokne in middleware', token);
    if (!token) {
        return res.status(401), res.send({ message: 'no access ' })
    }
...
async function run() {
    try {
        ...
        app.get('/bookings', logger, verifyToken, async (req, res) => {
            ...
            console.log('user in the valid tokken', req.user);
            let query = {}
            if (req.query?.email) {
...
app.listen(port, () => {
    console.log(`CAR doc is running on ${port}`);
})
